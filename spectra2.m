...
h_stim=psd(spectrum.periodogram('rectangular'),stim_center(stim_center(1:round(cycles/freq_stim(1)*1e4),p,i,j)~=0,p,i,j),'Fs',Fs,'NFFT',NFFT);
...
%}
y0 = stim_center(stim_center(1:round(cycles/freq_stim(1)*1e4),p,i,j)~=0,p,i,j);     %windowing required for short signals
wn = hann(length(y0));
for i2 = 1:length(wn)
    y(i2) = wn(i2)*y0(i2);
end
...
h_stim=psd(spectrum.periodogram('hann'),Delta_in_center(Delta_in_center(1:round(cycles/freq_stim(p)*1e4),p,i,j)~=0,p,i,j),'Fs',Fs,'NFFT',NFFT);
...
y0 = Delta_in_center(Delta_in_center(1:round(cycles/freq_stim(p)*1e4),p,i,j)~=0,p,i,j);
...
h_stim=psd(spectrum.periodogram('hann'),input(input(1:round(cycles/freq_stim(p)*1e4),p,i,j)~=0,p,i,j),'Fs',Fs,'NFFT',NFFT);

L = length(input(Delta_in_center(1:round(cycles/freq_stim(p)*1e4),p,i,j)~=0,p,i,j));
%{
hfft  = fft(Delta_in_center(Delta_in_center(1:round(cycles/freq_stim(p)*1e4),p,i,j)~=0,p,i,j),NFFT)/L;
qfft_Delta(:,2)=2*hfft(1:NFFT/2+1);
qfft_Delta(:,1) = Fs/2*linspace(0,1,NFFT/2+1);
...
y0 = input(input(1:round(cycles/freq_stim(p)*1e4),p,i,j)~=0,p,i,j);
%{
wn = hann(length(y0));
for i2 = 1:length(wn)
    y(i2) = wn(i2)*y0(i2);
...
qfft_stim(:,1) = Fs/2*linspace(0,1,NFFT/2+1);
%}
y0 = stim_center(stim_center(1:round(cycles/freq_stim(1)*1e4),p,i,j)~=0,p,i,j);
wn = hann(length(y0));
for i2 = 1:length(wn)
    y(i2) = wn(i2)*y0(i2);
end
...
h_stim=psd(spectrum.periodogram('hann'),Delta_in_center(Delta_in_center(1:round(cycles/freq_stim(1)*1e4),p,i,j)~=0,p,i,j),'Fs',Fs,'NFFT',NFFT);
...
y0 = Delta_in_center(Delta_in_center(1:round(cycles/freq_stim(1)*1e4),p,i,j)~=0,p,i,j);
wn = hann(length(y0));
for i2 = 1:length(wn)
    y(i2) = wn(i2)*y0(i2);
end
...
for p = 1:numfreq
    ...
    [S,F,T,P] = spectrogram(stim_center(stim_center(1:round(cycles/freq_stim(1)*1e4),p,i,j)~=0,p,i,j),length(stim_center(stim_center(1:round(cycles/freq_stim(1)*1e4),p,i,j)~=0,p,i,j))/1,0,NFFT,Fs);
    psd_q1_stim(:,p,i,j)=F;
    %psd_q2_stim(:,p,i,j)=P;
...
k = [250 250 250 250 250 250 250 250];
k = k*1e-6;
...
for p = 1:numfreq
clear y3 y4    
y3 = stim_center(stim_center(1:round(cycles/freq_stim(1)*1e4),p,i,j)~=0,p,i,j);
...
clear y3 y4    
y3 = Delta_in_center(Delta_in_center(1:round(cycles/freq_stim(1)*1e4),p,i,j)~=0,p,i,j);
...
warning off
  ...
  freq =freq_stim;
  i = 1;
  t = linspace(0,3,length(Delta_in_center(Delta_in_center(1:round(cycles/freq_stim(1)*1e4),1,1,1)~=0,p,i,j)));
  for i = 1:length(freq)
...
%%
  ...
  freq =freq_stim;
  i = 1;
  yt=stim_center(stim_center(1:round(cycles/freq_stim(1)*1e4),p,i,j)~=0,p,i,j);
...
for p = 1:numfreq
    ...
    yt=stim_center(stim_center(1:round(cycles/freq_stim(1)*1e4),p,i,j)~=0,p,i,j);
    yt2 = stim_center(1:length(yt),p,i,j);
    
    [Pxy,f] = cpsd(yt2,Delta_in_center(1:length(yt),p,i,j),[],0,NFFT,Fs);
    
    q=findnearest(f,freq_stim(p));
    phase_diff_CPSD(p,i,j)=angle(Pxy(q));
    
    [Pxy2,f2] = cpsd(yt2,yw(1:length(yt),p),[],0,NFFT,Fs);
    
    q=findnearest(f2,freq_stim(p));
    phase_diff_CPSD2(p,i,j)=angle(Pxy2(q));
